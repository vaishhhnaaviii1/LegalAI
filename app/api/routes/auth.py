...
# ==========================================
@router.post(
    "/signup", status_code=status.HTTP_201_CREATED, summary="Register a new user"
)
async def create_user(request: UserCreate, db: AsyncSession = Depends(get_db_session)):
...
async def db_login(request: LoginRequest, db: AsyncSession = Depends(get_db_session)):

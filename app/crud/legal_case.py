...
class CRUDLegalCase(CRUDBase[LegalCase, CaseRequest, CaseUpdate]):
    async def get_multi_by_user(
        ...
        *,  # *: Enforces keyword-only arguments.
        ...
        search: (
            str | None
        ) = None,  # If the React frontend doesn't send a search word, it just fetches all cases normally.
    ) -> List[LegalCase]:
        ...
        query = (
            query.options(selectinload(self.model.sections)).offset(skip).limit(limit)
        )

...
class ChargesActionRequest(BaseModel):
    approved_id: list[UUID] | None = Field(
        None, description="List of IDs for charges that the lawyer approved (Ticked)."
    )
...
class LegalSection(BaseModel):
    ipc_section: str = Field(
        description="The applicable Indian Penal Code (IPC) section, e.g., 'Section 378'"
    )
    bns_equivalent: str = Field(
        description="The equivalent section in the Bharatiya Nyaya Sanhita (BNS), e.g., 'Section 303(2)'"
    )
